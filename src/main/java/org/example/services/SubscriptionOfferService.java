...
public class SubscriptionOfferService {
    ...
    public void ajouter(SubscriptionOffer o) throws SQLException {
        try (Connection c = fresh()) {
            ...
            ps.setString(2, o.getName()); ps.setInt(3, o.getDurationMonths());
            ps.setDouble(4, o.getPrice()); ps.setString(5, o.getDescription());
            ps.executeUpdate();
    ...
    public List<SubscriptionOffer> afficher() throws SQLException {
        try (Connection c = fresh()) {
            ...
            while (rs.next()) {
                ...
                o.setName(rs.getString("name")); o.setDurationMonths(rs.getInt("duration_months"));
                o.setPrice(rs.getDouble("price")); o.setDescription(rs.getString("description"));
                o.setActive(rs.getInt("is_active") == 1);
                o.setCreatedAt(rs.getTimestamp("created_at")); o.setUpdatedAt(rs.getTimestamp("updated_at"));
                o.setGymnasiumName(rs.getString("gym_name"));
                list.add(o);
            }
            return list;
        }
    ...
    public void modifier(SubscriptionOffer o) throws SQLException {
        try (Connection c = fresh()) {
            ...
            if (o.getGymnasiumId() != null) ps.setInt(1, o.getGymnasiumId()); else ps.setNull(1, Types.INTEGER);
            ps.setString(2, o.getName()); ps.setInt(3, o.getDurationMonths());
            ps.setDouble(4, o.getPrice()); ps.setString(5, o.getDescription()); ps.setInt(6, o.getId());
            ps.executeUpdate();
        }
    }
    ...
    public void toggleActive(int id, boolean active) throws SQLException {
        try (Connection c = fresh()) {
            PreparedStatement ps = c.prepareStatement("UPDATE gym_subscription_offers SET is_active=?, updated_at=NOW() WHERE id=?");
    ...
    public void supprimer(int id) throws SQLException {
        try (Connection c = fresh()) {
            PreparedStatement ps = c.prepareStatement("DELETE FROM gym_subscription_offers WHERE id=?");
    ...
    public List<String[]> getOrders(int offerId) throws SQLException {
        try (Connection c = fresh()) {
            ...
            while (rs.next()) list.add(new String[]{
                String.valueOf(rs.getInt("id")), String.valueOf(rs.getInt("user_id")),
                String.valueOf(rs.getInt("quantity")), String.format("%.2f", rs.getDouble("unit_price")),
                String.format("%.2f", rs.getDouble("total_price")), rs.getString("status"),
